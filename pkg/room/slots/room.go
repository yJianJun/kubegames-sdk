...
func (r *Room) Match(s server.Server, player *playerObject.Player) (int64, bool, error) {
	...
	//slots games create table
	if atomic.LoadInt64(&r.TableCount)*int64(room.MaxPeople) < int64(r.Config.MaxPeople) {
		...
		log.Tracef("match player %d  into solts games %d", player.PlayerID, table.GetID())
		return table.GetID(), true, nil
	}

	//match player fail
	return 0, false, errors.New("match error")
}
